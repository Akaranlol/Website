...
class MY_PostWidget extends WP_Widget {
  ...
  /** @see WP_Widget::widget */
						...
						<?php $wp_query = new WP_Query( $args ); ?>
								...
								<li class="clearfix">
									<?php if(has_post_thumbnail()) { ?>
										...
										$image = aq_resize( $img_url, 300, 179, true ); //resize & crop img
										?>
									...
									<?php } else { ?>
									    <?php get_template_part('includes/post-formats/post-meta'); ?>
									    <time class="alt" datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('j M, Y'); ?></time>
									<?php } ?>
                  <h4><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent Link to', 'theme1885');?> <?php the_title_attribute(); ?>"><?php the_title(); ?></a></h4>
									<?php if($excerpt_count!="") { ?>
									<div class="excerpt">
                  <?php $excerpt = get_the_excerpt(); echo my_string_limit_words($excerpt,$excerpt_count);?>
									</div>
									<?php } ?>
									<a href="<?php the_permalink() ?>" class="button-arrow"></a>
								</li>
								<?php endwhile; ?>
								</ul>
								<?php endif; ?>
								...
								<?php if($linkurl !=""){?>
									<div class="button-wrap"><a href="<?php echo $linkurl; ?>" class="button"><?php echo $linktext; ?></a></div>
								<?php } ?>
    ...
    function form($instance) {				
      ...
      <p><label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Category Slug:', 'theme1885'); ?> <input class="widefat" id="<?php echo $this->get_field_id('category'); ?>" name="<?php echo $this->get_field_name('category'); ?>" type="text" value="<?php echo $category; ?>" /></label></p>
			
			<p><label for="<?php echo $this->get_field_id('post_format'); ?>"><?php _e('Post format:', 'theme1885'); ?><br />
      ...
      <p><label for="<?php echo $this->get_field_id('sort_by'); ?>"><?php _e('Post order:', 'theme1885'); ?><br />
      ...
      <p><label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Posts per page:'); ?><input class="widefat" style="width:30px; display:block; text-align:center" id="<?php echo $this->get_field_id('count'); ?>" name="<?php echo $this->get_field_name('count'); ?>" type="text" value="<?php echo $count; ?>" /></label></p>
			...
			<p><label for="<?php echo $this->get_field_id('excerpt_count'); ?>"><?php _e('Excerpt length (words):'); ?><input class="widefat" style="width:30px; display:block; text-align:center" id="<?php echo $this->get_field_id('excerpt_count'); ?>" name="<?php echo $this->get_field_name('excerpt_count'); ?>" type="text" value="<?php echo $excerpt_count; ?>" /></label></p>
			 ...
			 <p><label for="<?php echo $this->get_field_id('linktext'); ?>"><?php _e('Link Text:', 'theme1885'); ?> <input class="widefat" id="<?php echo $this->get_field_id('linktext'); ?>" name="<?php echo $this->get_field_name('linktext'); ?>" type="text" value="<?php echo $linktext; ?>" /></label></p>
			 
			 <p><label for="<?php echo $this->get_field_id('linkurl'); ?>"><?php _e('Link Url:', 'theme1885'); ?> <input class="widefat" id="<?php echo $this->get_field_id('linkurl'); ?>" name="<?php echo $this->get_field_name('linkurl'); ?>" type="text" value="<?php echo $linkurl; ?>" /></label></p>
        <?php 
    }
